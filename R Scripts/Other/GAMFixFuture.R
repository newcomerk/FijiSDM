...
d <- c("GAM AUC for whole tuned Model", 0.975) ##insert AUC/ROC!!
e <- c("GAM TSS for whole tuned Model", 0.850) ##insert tss!!
...
save.image(paste0("../outputs_KN/models/futuremodels_",mydatasource,"_", myspecies, "_gam.RData"))
load(paste0("../outputs_KN/models/futuremodels_",mydatasource,"_", myspecies, "_gam.RData"))
...
write.csv(metadata, paste0("../data/species_occurrence_KN/Metadata_", myspecies, "_CodeInvF2.csv"), row.names = FALSE)
...
layersf_mod2 <- mask(crop(layersf, mod_region2), mod_region2)

e <- intersect(extent(ProtectedW), extent(layersf_mod2))
r1e <- crop(layersf_mod2, e)
...
layersf_mod2 <- stack(r1e, r3e)
names(layersf_mod2)
names(layersf_mod2)[12] <- 'Protected_Areas'
plot(layersf_mod2)

layersf_mod2<-stack(layersf_mod2)
#############################################
...
#if this raster is being mean, and doesn't change the scale. just divide by 1000
mod_projf85 <- mod_proj85 / 1000
plot(mod_projf85, col=clrs, breaks = brks, main =  paste0(myspecies, " GAM Future Projection RCP 8.5 2100"))
dev.copy(png, paste0("../outputs_KN/GAMTunedModel_FutureProjection85_", myspecies, ".png"))
dev.off()
...
layersf1_mod2 <- mask(crop(layersf1, mod_region2), mod_region2)

e <- intersect(extent(ProtectedW), extent(layersf1_mod2))
r1e <- crop(layersf1_mod2, e)
r3e <- resample(ProtectedW, r1e)
...
#layers_mod <- lapply(layers, crop, mod_region)
layersf1_mod2 <- stack(r1e, r3e)
names(layersf1_mod2)
names(layersf1_mod2)[12] <- 'Protected_Areas'
plot(layersf1_mod2)

layersf1_mod2<-stack(layersf1_mod2)
#############################################

layersf1_mod 
myExpl2<- layersf1_mod[[vars_sel]]
myExpl2 <- stack(myExpl2)
...
#future fiji
myExpl2<- layersf1_mod2[[vars_sel]]
myExpl2 <- stack(myExpl2)
...
#if this raster is being mean, and doesn't change the scale. just divide by 1000
mod_projf26 <- mod_proj26 / 1000
plot(mod_projf26, col=clrs, breaks = brks, main =  paste0(myspecies, " GAM Future Projection RCP 2.6 2100"))
dev.copy(png, paste0("../outputs_KN/GAMTunedModel_FutureProjection26_", myspecies, ".png"))
dev.off()
...
plot(layersf2)
layersf2_mod2 <- mask(crop(layersf2, mod_region2), mod_region2)

e <- intersect(extent(ProtectedW), extent(layersf2_mod2))
r1e <- crop(layersf2_mod2, e)
r3e <- resample(ProtectedW, r1e)
...
#layers_mod <- lapply(layers, crop, mod_region)
layersf2_mod2 <- stack(r1e, r3e)
names(layersf2_mod2)
names(layersf2_mod2)[12] <- 'Protected_Areas'
plot(layersf2_mod2)

layersf2_mod2<-stack(layersf2_mod2)
#############################################

layersf2_mod 
myExpl2<- layersf2_mod[[vars_sel]]
myExpl2 <- stack(myExpl2)
...
#future fiji
myExpl2<- layersf2_mod2[[vars_sel]]
myExpl2 <- stack(myExpl2)
...
#if this raster is being mean, and doesn't change the scale. just divide by 1000
mod_projf45 <- mod_proj45 / 1000
plot(mod_projf45, col=clrs, breaks = brks, main =  paste0(myspecies, " GAM Future Projection RCP 4.5 2100"))
dev.copy(png, paste0("../outputs_KN/GAMTunedModel_FutureProjection45_", myspecies, ".png"))
dev.off()
