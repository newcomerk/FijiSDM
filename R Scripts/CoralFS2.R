...
options(sdmpredictors_datadir = "../data/sdmpredictorsCoralFut")
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersfs <- load_layers(layers_choice_crlfs, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersfs  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_crlfs))
...
# define folder for downloading / fetching the variables' map layers:
options(sdmpredictors_datadir = "../data/sdmpredictorsCoralFut1")
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersfs1 <- load_layers(layers_choice_crlfs1, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersfs1  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_crlfs1))
metadata<-rbind(metadata, a)
...
# define folder for downloading / fetching the variables' map layers:
options(sdmpredictors_datadir = "../data/sdmpredictorsCoralFut2")
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersfs2 <- load_layers(layers_choice_crlfs2, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersfs2  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_crlfs2))
metadata<-rbind(metadata, a)
