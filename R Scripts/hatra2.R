...
write.csv(dat, paste0("../data/species_occurrence_KN/occurrences_", myspecies, "_finaldf.csv"), row.names = FALSE)
...
p_coords <- p_coords[ -c(1,4:34) ]
bg_coords <- dat[dat$presence == 0, ]
bg_coords <- bg_coords[ -c(1,4:34) ]
...
plot(map, col = clrs, breaks=brks, main = paste0(myspecies, " Maxent"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_", myspecies, ".png"))
dev.off()
...
plot(map.brt, col = clrs, breaks=brks, main = paste0(myspecies, " BRT"))
dev.copy(png, paste0("../outputs_KN/BRTTunedModel_", myspecies, ".png"))
dev.off()
...
d <- c("GAM AUC for whole tuned Model", 0.990) ##insert AUC/ROC!!
e <- c("GAM TSS for whole tuned Model", 0.946) ##insert tss!!
metadata<-rbind(metadata, d, e)
...
plot(mod_projf, col=clrs, breaks = brks, main =  paste0(myspecies, " GAM Present Day"))
...
# save all current objects to disk (YOU WILL NEED THEM LATER!):
save.image(paste0("../outputs_KN/models/presentmodels_",mydatasource,"_", myspecies, ".RData"))
...
varimp_gam<-as.data.frame(em.vi.list$Holothuria.atra_AllData_Full_GAM$mat)
...
e <- intersect(intersect(extent(ProtectedW), extent(fD2)), extent(layers_modf))
r1e <- crop(layers_modf, e)
...
layers_modf <- stack(r1e, r2e)
layers_modf <- stack(layers_modf, r3e)
names(layers_modf)[16] <- 'Dist_City'
names(layers_modf)[17] <- 'Protected_Areas'
plot(layers_modf)

data.tune.f <- prepareSWD(species = myspecies, p = p_coords, a = bg_coords,
                        env = layers_modf[[vars_sel]])

#maxent
pred.max.fut <- predict(model.final.max, data = data.tune.f, type = "cloglog")
map.fut <- predict(model.final.max, data = layers_modf[[vars_sel]], type = "cloglog")
plot(map.fut, col = clrs, main = paste0(myspecies, "Maxent Future Projection"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_FutureProjection", myspecies, ".png"))
dev.off()

#brt
pred.brt.fut <- predict(model.final.brt, data = data.tune.f, type = "cloglog")
map.fut <- predict(model.final.brt, data = layers_modf[[vars_sel]], type = "cloglog")
plot(map.fut, col = clrs, main = paste0(myspecies, "BRT Future Projection"))
dev.copy(png, paste0("../outputs_KN/BRTTunedModel_FutureProjection", myspecies, ".png"))
dev.off()
...
for (f in folds) {
  ...
  dat_train <- subset(dat, foldIDr != f)
  ...
  dat[ , paste0("gam_fold", f, "_pred")] <- predict(gam_mod_fold, newdata = dat, type = "response")
  
  maxent_mod_fold <- maxent(x = dat_train[ , vars_sel], p = dat_train[ , "presence"])
...
# save the dat for dani to see issues
#write.csv(dat, paste0("../outputs_KN/dat_", myspecies, "error10june21.csv"), row.names = FALSE)

# EVALUATE EACH MODEL ON ITS VALIDATION FOLD ####

fold_cols <- grep("_fold", names(dat))
names(dat)[fold_cols]
models <- c("gam", "maxent", "maxnet", "gbm")  # "bioclim", "domain", "maxent"
#models <- c( "maxent",  "gbm")  # "bioclim", "domain", "maxent"
...
write.csv(crossval, paste0("../outputs_KN/crossval_", mydatasource,"_", myspecies, ".csv"), row.names = FALSE)
