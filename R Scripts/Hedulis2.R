...
model <- SDMtune::train(method = "Maxent", data = data.tune)
...
model <- SDMtune::train("Maxent", data = train)
...
model.final.max <- SDMtune::train(method = "Maxent", data = data.tune, fc = fc, reg = reg, iter = 500)
...
cv_model <- SDMtune::train("Maxent", data = data.tune, fc = fc, reg = reg, folds = folds)
cv_model
#mean of training
auc(cv_model)
#mean of testing
...
###########BRT
model2 <- SDMtune::train("BRT", data = train)
...
model.final.brt <- SDMtune::train(method = "BRT", data = data.tune, n.trees = ntree, shrinkage = shrink)
...
library(biomod2)
 ...
 # ISSUE: do we load only presence points? gam needs absences so need to do something here.
 
 myResp <- as.numeric(dat$presence)
 myResp<-myResp[myResp == 1]

  # the XY coordinates of species data
 x<-dat[dat$presence == 1, ]
 myRespXY <- x[,c("x","y")]
...
plot(myBiomodData)
BIOMOD_tuning(myBiomodData, models = "GAM", models.options = BIOMOD_ModelingOptions())

select <- TRUE   ## add number from above
method <-  "GCV.Cp"   ## add number from above
c <-c("GAM select", select)
e <-c("GAM method (not changed)", method)
metadata<-rbind(metadata, c,e)



DataSplitTable <- BIOMOD_cv(myBiomodData, k=5, do.full.models= F)

# 4. Doing Modelisation

myBiomodModelOut <- BIOMOD_Modeling( myBiomodData, 
                                     models = c('GAM'), 
                                     models.options = BIOMOD_ModelingOptions(GAM = list(method = method, select = select)), 
