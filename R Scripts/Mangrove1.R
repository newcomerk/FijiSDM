...
foo <- readOGR(paste0("../../Mangrove GIS/mang_points.shp"))
occurrences <- as(foo, "data.frame")

#mangrove_occurences <- st_read(paste0("../../Mangrove GIS/mang_points.shp"))
...
####################cropping first because so slow
...
plot(occurrences_spatial, main = paste0(myspecies))
# add countries map:
plot(countries, border = "blue", lwd = 2, add = TRUE)
map.axes()
dev.copy(png, paste0("../outputs_KN/EdittedPoints_FinalWindow", myspecies, ".png"))
dev.off()
...
write.csv(df, paste0("../data/species_occurrence_KN/occurrences_", myspecies, "_cropped.csv"), row.names = FALSE)

# from now on, you don't need to download and clean the data again - you can just import them from the .csv:
occurrences <- read.csv(paste0("../data/species_occurrence_KN/occurrences_", myspecies, "_cropped.csv"))
...
layers_choice_mng <- c("WC_bio2", "WC_bio4", "WC_bio5", "WC_bio6", "WC_bio14", "WC_bio18", "WC_bio19", "WC_alt")
#mngp is present compared to future
layers_choice_mngp <- c("WC_bio2", "WC_bio4", "WC_bio5", "WC_bio6", "WC_bio14", "WC_bio18", "WC_bio19", "WC_alt")
#mangp is future to compare to present
layers_choice_mngf <- c("WC_bio2_cc85_2070", "WC_bio4_cc85_2070", "WC_bio5_cc85_2070", "WC_bio6_cc85_2070", "WC_bio14_cc85_2070", 
                        "WC_bio18_cc85_2070", "WC_bio19_cc85_2070", "WC_alt")

# define folder for downloading / fetching the variables' map layers:
options(sdmpredictors_datadir = "../data/sdmpredictorsMangrove")
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layers <- load_layers(layers_choice_mng, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layers  # a list of raster maps

# define folder for downloading / fetching the variables' map layers:
options(sdmpredictors_datadir = "../data/sdmpredictors_MangrovePres")
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layers <- load_layers(layers_choice_mngp, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layers  # a list of raster maps


# define folder for downloading / fetching the variables' map layers:
options(sdmpredictors_datadir = "../data/sdmpredictors_MangroveFut")
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layers <- load_layers(layers_choice_mngf, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layers  # a list of raster maps



a <- c("Downloaded Env Layers", toString(layers_choice_mng))
metadata<-rbind(metadata, a)
...
layers <- stack(layers)
plot(layers)
dev.copy(png, paste0("../outputs_KN/MangroveEnvLayers.png"))
dev.off()
...
mod_region <- as(extent(my_window), "SpatialPolygons")
occurrences_spatial<- crop(occurrences_spatial, mod_region)
plot(occurrences_spatial, main = paste0(myspecies, " Land"))
# add countries map:
plot(countries, border = "blue", lwd = 2, add = TRUE)
map.axes()
dev.copy(png, paste0("../outputs_KN/EdittedPoints_FinalWindow", myspecies, ".png"))
...
#Protected #land is not masked hopefully
#
#layers_mod <- mask(crop(layers, mod_region), mod_region)
#fD2 <- mask(crop(fD2, mod_region),mod_region)
#fijiland<- crop(countries, mod_region)
#
#mean_dist <- mean(spDists(occurrences_spatial))
#mean_dist  # in km
#pres_buff <- raster::buffer(fijiland, width = .05, dissolve = TRUE)
#plot(pres_buff, lwd = 2)
#plot(occurrences_spatial, col = "blue", add = TRUE)
#plot(countries, border = "red", add = TRUE)
#
#fD2 <- mask(crop(fD2, pres_buff),pres_buff)
#Protected <- mask(crop(Protected, pres_buff),pres_buff)
#
#e <- intersect(intersect(extent(Protected), extent(fD2)), extent(layers_mod))
#r1e <- crop(layers_mod, e)
#r2e <- resample(fD2, r1e)
#r3e <- resample(Protected, r1e)
#
...
#names(layers_mod)[9] <- 'Dist_City'
#names(layers_mod)[10] <- 'Protected_Areas'
plot(layers_mod)
...
metadata<-rbind(metadata, b,c,d,e)
