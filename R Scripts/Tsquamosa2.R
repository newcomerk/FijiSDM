...
#vars_sel<-as.list(strsplit(metadata$c..Tridacna.squamosa....300..[18] , ", "))
...
d <- c("Maxent AUC for Entire tuned Model", auc(model.final.max))
e <- c("Maxent TSS for Entire tuned Model", tss(model.final.max))
...
d <- c("BRT AUC for Entire tuned Model", auc(model.final.brt))
e <- c("BRT TSS for Entire tuned Model", tss(model.final.brt))
metadata<-rbind(metadata, d, e)
#but lets check with a cross-validation
...
time.seq<-system.time(Biomod.tuning <- BIOMOD_tuning(myBiomodData, models = "GAM",
                                                     env.ME = myExpl,
...
d <- c("GAM AUC for whole tuned Model", 0.994) ##insert AUC/ROC!!
e <- c("GAM TSS for whole tuned Model", 0.972) ##insert tss!!
metadata<-rbind(metadata, d, e)
...
map.fut <- predict(model.final.max, data = layersf_mod[[vars_sel]], type = "cloglog")
...
map.fut <- predict(model.final.brt, data = layersf_mod[[vars_sel]], type = "cloglog")
