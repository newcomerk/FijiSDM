...
vars <- names(dat)[grep("WC|Dist|Protected", names(dat))]
...
options(sdmpredictors_datadir = "../data/sdmpredictorsMangroveFut")
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersf <- load_layers(layers_choice_mngf, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersf  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_inf))
metadata<-rbind(metadata, a)

names(layersf) <- c("WC_bio2", "WC_bio4", "WC_bio5", "WC_bio6", "WC_bio14", "WC_bio18", "WC_bio19", "WC_alt")
...
# define folder for downloading / fetching the variables' map layers:
options(sdmpredictors_datadir = "../data/sdmpredictorsMangroveFut1")
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersf1 <- load_layers(layers_choice_mngf1, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersf1  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_inf1))
metadata<-rbind(metadata, a)

names(layersf1) <- c("WC_bio2", "WC_bio4", "WC_bio5", "WC_bio6", "WC_bio14", "WC_bio18", "WC_bio19", "WC_alt")
...
# define folder for downloading / fetching the variables' map layers:
options(sdmpredictors_datadir = "../data/sdmpredictorsMangroveFut2")
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersf2 <- load_layers(layers_choice_mngf2, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersf2  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_inf2))
metadata<-rbind(metadata, a)

names(layersf2) <- c("WC_bio2", "WC_bio4", "WC_bio5", "WC_bio6", "WC_bio14", "WC_bio18", "WC_bio19", "WC_alt")
