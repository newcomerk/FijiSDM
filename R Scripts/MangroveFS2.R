...
vars <- names(dat)[grep("WC|BO|MS|Dist|Protected", names(dat))]
...
layersfs <- load_layers(layers_choice_mngfs, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersfs  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_mngfs))
metadata<-rbind(metadata, a)

names(layersfs) <- c("WC_bio2", "WC_bio4", "WC_bio5", "WC_bio6", "WC_bio14", "WC_bio18", "WC_bio19")
...
AltFuture<-raster("../data/AltitudeFuture.tif")

layersfs_mod<- stack(layersfs_mod, AltFuture)

names(layersfs_mod)
names(layersfs_mod)[8] <- 'WC_alt'
plot(layersfs_mod)
...
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersfs1 <- load_layers(layers_choice_mngfs1, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersfs1  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_mngfs1))
metadata<-rbind(metadata, a)

names(layersfs1) <- c("WC_bio2", "WC_bio4", "WC_bio5", "WC_bio6", "WC_bio14", "WC_bio18", "WC_bio19")
...
layersfs1_mod<- stack(layersfs1_mod, AltFuture)


names(layersfs1_mod)
names(layersfs1_mod)[8] <- 'WC_alt'
plot(layersfs1_mod)
...
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersfs2 <- load_layers(layers_choice_mngfs2, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersfs2  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_mngfs2))
metadata<-rbind(metadata, a)

names(layersfs2) <- c("WC_bio2", "WC_bio4", "WC_bio5", "WC_bio6", "WC_bio14", "WC_bio18", "WC_bio19")
...
layersfs2_mod<- stack(layersfs2_mod, AltFuture)

names(layersfs2_mod)
names(layersfs2_mod)[8] <- 'WC_alt'
plot(layersfs2_mod)
