...
map <- predict(model.final.max, data = layers_mod2[[vars_sel]], type = "cloglog")
...
ntree <- 6000   ## add number from above
shrink <-  0.083   ## add number from above
...
map.brt <- predict(model.final.brt, data = layers_mod2[[vars_sel]])
...
#############################make fiji projection
my_window2 <- c(176, 180, -21, -14)
#just Fiji
mod_region2 <- as(extent(my_window2), "SpatialPolygons")
ProtectedW<-raster("../data/ProtectedCropped.tif")
#fD2<-raster("../data/DistanceCity.tif")

layersfs_mod2 <- mask(crop(layersfs, mod_region2), mod_region2)


BathyMaxFuture<-raster("../data/BathyMaxFuture.tif")
BathyFuture<-raster("../data/BathyFuture.tif")

layersfs_mod2<- stack(layersfs_mod2, BathyMaxFuture, BathyFuture)

e <- intersect(extent(ProtectedW), extent(layersfs_mod2))
r1e <- crop(layersfs_mod2, e)
r3e <- resample(ProtectedW, r1e)

t(sapply(c(r1e, r3e), function(i) as.vector(extent(i))))

#layers_mod <- lapply(layers, crop, mod_region)
layersfs_mod2 <- stack(r1e, r3e)
names(layersfs_mod2)
names(layersfs_mod2)[10] <- 'BO_bathymax'
names(layersfs_mod2)[11] <- 'MS_bathy_5m'
names(layersfs_mod2)[12] <- 'Protected_Areas'
plot(layersfs_mod2)
...
BathyMaxFutureAUS<-raster("../data/BathyMaxFutureAUS.tif")
BathyFutureAUS<-raster("../data/BathyFutureAUS.tif")

layersfs_mod<- stack(layersfs_mod, BathyMaxFutureAUS, BathyFutureAUS)

e <- intersect(extent(ProtectedW), extent(layersfs_mod))
...
#maxent
pred.max.fut <- predict(model.final.max, data = data.tune.f, type = "cloglog")
map.max.fut <- predict(model.final.max, data = layersfs_mod[[vars_sel]], type = "cloglog")
plot(map.max.fut, col = clrs , breaks = brks, main = paste0(myspecies, " Maxent Future Projection AUS RCP 8.5 2100 w SLR"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_AUS_FutureProjection85_SLR", myspecies, ".png"))
dev.off()

map.max.fut <- predict(model.final.max, data = layersfs_mod2[[vars_sel]], type = "cloglog")
plot(map.max.fut, col = clrs , breaks = brks, main = paste0(myspecies, " Maxent Future Projection RCP 8.5 2100 w SLR"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_FutureProjection85_SLR", myspecies, ".png"))
dev.off()

#brt
pred.brt.fut <- predict(model.final.brt, data = data.tune.f, type = "cloglog")
map.brt.fut <- predict(model.final.brt, data = layersfs_mod[[vars_sel]], type = "cloglog")
plot(map.brt.fut, col = clrs, breaks = brks, main = paste0(myspecies, " BRT Future Projection AUS RCP 8.5 2100 w SLR"))
dev.copy(png, paste0("../outputs_KN/BRTTunedModel_AUS_FutureProjection85_SLR", myspecies, ".png"))
dev.off()

map.brt.fut <- predict(model.final.brt, data = layersfs_mod2[[vars_sel]], type = "cloglog")
plot(map.brt.fut, col = clrs, breaks = brks, main = paste0(myspecies, " BRT Future Projection RCP 8.5 2100 w SLR"))
...
layersfs1_mod2 <- mask(crop(layersfs1, mod_region2), mod_region2)

BathyMaxFuture<-raster("../data/BathyMaxFuture.tif")
BathyFuture<-raster("../data/BathyFuture.tif")

layersfs1_mod2<- stack(layersfs1_mod2, BathyMaxFuture, BathyFuture)


e <- intersect(extent(ProtectedW), extent(layersfs1_mod2))
r1e <- crop(layersfs1_mod2, e)
r3e <- resample(ProtectedW, r1e)

t(sapply(c(r1e, r3e), function(i) as.vector(extent(i))))

#layers_mod <- lapply(layers, crop, mod_region)
layersfs1_mod2 <- stack(r1e, r3e)
names(layersfs1_mod2)[10] <- 'BO_bathymax'
names(layersfs1_mod2)[11] <- 'MS_bathy_5m'
names(layersfs1_mod2)[12] <- 'Protected_Areas'
plot(layersfs1_mod2)
...
layersfs1_mod<- stack(layersfs1_mod, BathyMaxFutureAUS, BathyFutureAUS)

e <- intersect(extent(ProtectedW), extent(layersfs1_mod))
...
#maxent
pred.max.fut <- predict(model.final.max, data = data.tune.f, type = "cloglog")
map.max.fut <- predict(model.final.max, data = layersfs1_mod[[vars_sel]], type = "cloglog")
plot(map.max.fut, col = clrs , breaks = brks, main = paste0(myspecies, " Maxent Future Projection AUS RCP 2.6 2100 w SLR"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_AUS_FutureProjection26_SLR", myspecies, ".png"))
dev.off()

map.max.fut <- predict(model.final.max, data = layersfs1_mod2[[vars_sel]], type = "cloglog")
plot(map.max.fut, col = clrs , breaks = brks, main = paste0(myspecies, " Maxent Future Projection RCP 2.6 2100 w SLR"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_FutureProjection26_SLR", myspecies, ".png"))
dev.off()

#brt
pred.brt.fut <- predict(model.final.brt, data = data.tune.f, type = "cloglog")
map.brt.fut <- predict(model.final.brt, data = layersfs1_mod[[vars_sel]], type = "cloglog")
plot(map.brt.fut, col = clrs, breaks = brks, main = paste0(myspecies, " BRT Future Projection AUS RCP 2.6 2100 w SLR"))
dev.copy(png, paste0("../outputs_KN/BRTTunedModel_AUS_FutureProjection26_SLR", myspecies, ".png"))
dev.off()

map.brt.fut <- predict(model.final.brt, data = layersfs1_mod2[[vars_sel]], type = "cloglog")
plot(map.brt.fut, col = clrs, breaks = brks, main = paste0(myspecies, " BRT Future Projection RCP 2.6 2100 w SLR"))
...
layersfs2_mod2 <- mask(crop(layersfs2, mod_region2), mod_region2)

BathyMaxFuture<-raster("../data/BathyMaxFuture.tif")
BathyFuture<-raster("../data/BathyFuture.tif")

layersfs2_mod2<- stack(layersfs2_mod2, BathyMaxFuture, BathyFuture)

e <- intersect(extent(ProtectedW), extent(layersfs2_mod2))
r1e <- crop(layersfs2_mod2, e)
r3e <- resample(ProtectedW, r1e)

t(sapply(c(r1e, r3e), function(i) as.vector(extent(i))))

#layers_mod <- lapply(layers, crop, mod_region)
layersfs2_mod2 <- stack(r1e, r3e)
names(layersfs2_mod2)[10] <- 'BO_bathymax'
names(layersfs2_mod2)[11] <- 'MS_bathy_5m'
names(layersfs2_mod2)[12] <- 'Protected_Areas'
plot(layersfs2_mod2)
...
layersfs2_mod<- stack(layersfs2_mod, BathyMaxFutureAUS, BathyFutureAUS)

e <- intersect(extent(ProtectedW), extent(layersfs2_mod))
...
#maxent
pred.max.fut <- predict(model.final.max, data = data.tune.f, type = "cloglog")
map.max.fut <- predict(model.final.max, data = layersfs2_mod[[vars_sel]], type = "cloglog")
plot(map.max.fut, col = clrs , breaks = brks, main = paste0(myspecies, " Maxent Future Projection AUS RCP 4.5 2100 w SLR"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_AUS_FutureProjection45_SLR", myspecies, ".png"))
dev.off()

map.max.fut <- predict(model.final.max, data = layersfs2_mod2[[vars_sel]], type = "cloglog")
plot(map.max.fut, col = clrs , breaks = brks, main = paste0(myspecies, " Maxent Future Projection RCP 4.5 2100 w SLR"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_FutureProjection45_SLR", myspecies, ".png"))
dev.off()

#brt
pred.brt.fut <- predict(model.final.brt, data = data.tune.f, type = "cloglog")
map.brt.fut <- predict(model.final.brt, data = layersfs2_mod[[vars_sel]], type = "cloglog")
plot(map.brt.fut, col = clrs, breaks = brks, main = paste0(myspecies, " BRT Future Projection AUS RCP 4.5 2100 w SLR"))
dev.copy(png, paste0("../outputs_KN/BRTTunedModel_AUS_FutureProjection45_SLR", myspecies, ".png"))
dev.off()

map.brt.fut <- predict(model.final.brt, data = layersfs2_mod2[[vars_sel]], type = "cloglog")
plot(map.brt.fut, col = clrs, breaks = brks, main = paste0(myspecies, " BRT Future Projection RCP 4.5 2100 w SLR"))
