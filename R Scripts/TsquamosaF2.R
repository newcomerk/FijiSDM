...
dat <- read.csv(paste0("../outputs_KN/dat_present_", myspecies, ".csv"))
# or (depending on which dataset you want to use):
#dat <- read.csv(paste0("../outputs_KN/dat_grid_", myspecies, ".csv"))
head(dat)
layers_mod <- readRDS(paste0("../outputs_KN/sdmpredictors_mod_present_", myspecies, ".rds"))
plot(layers_mod)
...
save.image(paste0("../outputs_KN/models_",mydatasource,"_", myspecies, ".RData"))
...
load(paste0("../outputs_KN/models_",mydatasource,"_", myspecies, ".RData"))
...
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_Present_", myspecies, ".png"))
dev.off()
...
ntree <- 1000   ## add number from above
shrink <-  0.012   ## add number from above
...
dev.copy(png, paste0("../outputs_KN/BRTTunedModel_Present_", myspecies, ".png"))
dev.off()
...
layersf <- load_layers(layers_choice_inf, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersf  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_inf))
metadata<-rbind(metadata, a)

names(layersf) <- c("BO2_salinitymean_bdmax", "BO2_tempmin_bdmax", "BO2_tempmax_bdmax", "BO2_temprange_bdmax",
...
layersf <- stack(layersf)
plot(layersf)

layersf_mod <- mask(crop(layersf, mod_region), mod_region)
fD2 <- mask(crop(fD2, mod_region),mod_region)


e <- intersect(intersect(extent(ProtectedW), extent(fD2)), extent(layersf_mod))
r1e <- crop(layersf_mod, e)
...
layersf_mod <- stack(r1e, r2e)
layersf_mod <- stack(layersf_mod, r3e)
names(layersf_mod)
names(layersf_mod)[12] <- 'Dist_City'
names(layersf_mod)[13] <- 'Protected_Areas'
plot(layersf_mod)

data.tune.f <- prepareSWD(species = myspecies, p = p_coords, a = bg_coords,
                          env = layersf_mod[[vars_sel]])
...
plot(map.max.fut, col = clrs , breaks = brks, main = paste0(myspecies, " Maxent Future Projection RCP 8.5 2100"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_FutureProjection85", myspecies, ".png"))
dev.off()
...
plot(map.brt.fut, col = clrs, breaks = brks, main = paste0(myspecies, " BRT Future Projection RCP 8.5 2100"))
dev.copy(png, paste0("../outputs_KN/BRTTunedModel_FutureProjection85", myspecies, ".png"))
dev.off()
...
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersf1 <- load_layers(layers_choice_inf1, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersf1  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_inf1))
metadata<-rbind(metadata, a)

names(layersf1) <- c("BO2_salinitymean_bdmax", "BO2_tempmin_bdmax", "BO2_tempmax_bdmax", "BO2_temprange_bdmax",
                     "BO21_curvelmax_bdmax", "BO_bathymax",
                     "MS_biogeo06_bathy_slope_5m", "MS_biogeo01_aspect_EW_5m", "MS_bathy_5m", "MS_biogeo05_dist_shore_5m",
                     "MS_biogeo02_aspect_NS_5m")

layersf1 <- stack(layersf1)
plot(layersf1)

layersf1_mod <- mask(crop(layersf1, mod_region), mod_region)
fD2 <- mask(crop(fD2, mod_region),mod_region)

e <- intersect(intersect(extent(ProtectedW), extent(fD2)), extent(layersf1_mod))
r1e <- crop(layersf1_mod, e)
r2e <- resample(fD2, r1e)
r3e <- resample(ProtectedW, r1e)
...
#layers_mod <- lapply(layers, crop, mod_region)
layersf1_mod <- stack(r1e, r2e)
layersf1_mod <- stack(layersf1_mod, r3e)
names(layersf1_mod)
names(layersf1_mod)[12] <- 'Dist_City'
names(layersf1_mod)[13] <- 'Protected_Areas'
plot(layersf1_mod)

data.tune.f <- prepareSWD(species = myspecies, p = p_coords, a = bg_coords,
                          env = layersf1_mod[[vars_sel]])

#maxent
pred.max.fut <- predict(model.final.max, data = data.tune.f, type = "cloglog")
map.max.fut <- predict(model.final.max, data = layersf1_mod[[vars_sel]], type = "cloglog")
plot(map.max.fut, col = clrs , breaks = brks, main = paste0(myspecies, " Maxent Future Projection RCP 2.6 2100"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_FutureProjection26", myspecies, ".png"))
dev.off()

#brt
pred.brt.fut <- predict(model.final.brt, data = data.tune.f, type = "cloglog")
map.brt.fut <- predict(model.final.brt, data = layersf1_mod[[vars_sel]], type = "cloglog")
plot(map.brt.fut, col = clrs, breaks = brks, main = paste0(myspecies, " BRT Future Projection RCP 2.6 2100"))
dev.copy(png, paste0("../outputs_KN/BRTTunedModel_FutureProjection26", myspecies, ".png"))
dev.off()
...
# load the layers to the current R session (downloading them if they aren't already in the folder defined above):
layersf2 <- load_layers(layers_choice_inf2, rasterstack = FALSE)  # rasterstack=TRUE gives error when there are layers with different extent
layersf2  # a list of raster maps

a <- c("Downloaded Future Env Layers", toString(layers_choice_inf2))
metadata<-rbind(metadata, a)

names(layersf2) <- c("BO2_salinitymean_bdmax", "BO2_tempmin_bdmax", "BO2_tempmax_bdmax", "BO2_temprange_bdmax",
                     "BO21_curvelmax_bdmax", "BO_bathymax",
                     "MS_biogeo06_bathy_slope_5m", "MS_biogeo01_aspect_EW_5m", "MS_bathy_5m", "MS_biogeo05_dist_shore_5m",
                     "MS_biogeo02_aspect_NS_5m")

layersf2 <- stack(layersf2)
plot(layersf2)

layersf2_mod <- mask(crop(layersf2, mod_region), mod_region)
fD2 <- mask(crop(fD2, mod_region),mod_region)

e <- intersect(intersect(extent(ProtectedW), extent(fD2)), extent(layersf2_mod))
r1e <- crop(layersf2_mod, e)
r2e <- resample(fD2, r1e)
r3e <- resample(ProtectedW, r1e)
...
#layers_mod <- lapply(layers, crop, mod_region)
layersf2_mod <- stack(r1e, r2e)
layersf2_mod <- stack(layersf2_mod, r3e)
names(layersf2_mod)
names(layersf2_mod)[12] <- 'Dist_City'
names(layersf2_mod)[13] <- 'Protected_Areas'
plot(layersf2_mod)

data.tune.f <- prepareSWD(species = myspecies, p = p_coords, a = bg_coords,
                          env = layersf2_mod[[vars_sel]])

#maxent
pred.max.fut <- predict(model.final.max, data = data.tune.f, type = "cloglog")
map.max.fut <- predict(model.final.max, data = layersf2_mod[[vars_sel]], type = "cloglog")
plot(map.max.fut, col = clrs , breaks = brks, main = paste0(myspecies, " Maxent Future Projection RCP 4.5 2100"))
dev.copy(png, paste0("../outputs_KN/MaxentTunedModel_FutureProjection45", myspecies, ".png"))
dev.off()

#brt
pred.brt.fut <- predict(model.final.brt, data = data.tune.f, type = "cloglog")
map.brt.fut <- predict(model.final.brt, data = layersf2_mod[[vars_sel]], type = "cloglog")
plot(map.brt.fut, col = clrs, breaks = brks, main = paste0(myspecies, " BRT Future Projection RCP 4.5 2100"))
dev.copy(png, paste0("../outputs_KN/BRTTunedModel_FutureProjection45", myspecies, ".png"))
dev.off()
...
########################################################################
# save all current objects to disk (YOU WILL NEED THEM LATER!): 
save.image(paste0("../outputs_KN/models/futuremodels_",mydatasource,"_", myspecies, ".RData")) 
...
write.csv(metadata, paste0("../data/species_occurrence_KN/Metadata_", myspecies, "_CodeInvF.csv"), row.names = FALSE)
