...
my_window <- c(144, 150, -21, -10)
#just Australia


################!!!!!!!!!!!! Tgigas has no occurences in fiji, so mapping australia. things will be different
...
#Fd2 was not made to be this large
#fD2 <- mask(crop(fD2, mod_region),mod_region)


e <- intersect(extent(ProtectedW), extent(layers_mod))
...
layers_mod <- stack(r1e, r3e)
names(layers_mod)
names(layers_mod)[12] <- 'Protected_Areas'
plot(layers_mod)
